<h1 align="center">Hi, I'm Kea Meng 👋</h1>

<p align="center">
  <strong>Full-Stack Product Developer</strong> based in Phnom Penh, Cambodia
</p>

<p align="center">
  <a href="https://www.keameng-ing.com/">Portfolio</a> ·
  <a href="https://www.linkedin.com/in/kea-meng-ing/">LinkedIn</a> ·
  <a href="mailto:keameng9@gmail.com">Email</a>
</p>

<p align="center">
  <a href="https://www.keameng-ing.com/"><img src="https://img.shields.io/badge/Portfolio-Live-7C3AED?style=flat-square&logo=vercel&logoColor=white" alt="Portfolio live" /></a>
  <a href="https://www.linkedin.com/in/kea-meng-ing/"><img src="https://img.shields.io/badge/LinkedIn-Let's_connect-0A66C2?style=flat-square&logo=linkedin&logoColor=white" alt="Connect on LinkedIn" /></a>
  <a href="mailto:keameng9@gmail.com"><img src="https://img.shields.io/badge/Available_for-Opportunities-16A34A?style=flat-square&logo=gmail&logoColor=white" alt="Available for opportunities" /></a>
  <img src="https://komarev.com/ghpvc/?username=KeaMeng-ING&style=flat-square&color=7C3AED&label=Profile+views" alt="Kea Meng's profile views" />
</p>

I turn ideas and business workflows into production-ready digital products, from product discovery and interface development to backend architecture, databases, and deployment. My core stack is **Next.js**, **NestJS**, **TypeScript**, and **PostgreSQL**, supported by Docker, Nginx, and CI/CD.

I'm completing my Software Development degree at the **American University of Phnom Penh** with a **3.73 GPA**. I currently freelance, co-founded **ShopiLink**, and am open to freelance projects, collaborations, and full-time opportunities worldwide.

## Featured work

### [ShopiLink — Order & Inventory Management](https://inventory.api.shopilinkcambodia.com/login)

Production SaaS for Cambodian wholesale SMEs, built as co-founder and technical lead. It brings multi-branch inventory, orders, invoices, delivery notes, reporting, async exports, and AI-assisted demand insights into one system.

`Next.js` `NestJS` `PostgreSQL` `Prisma` `Docker` `Nginx` `GitHub Actions`

### [Sabay Bike Tour — Booking & Operations](https://sabay-tour-azure.vercel.app/)

Full-stack client platform combining a public bicycle-tour booking journey with a role-based back office, live pricing, email confirmations, finance workflows, audit history, and automated daily closing.

`Next.js` `TypeScript` `Supabase` `PostgreSQL` `Prisma` `Tailwind CSS`

### [Learnify — AI Learning SaaS](https://learnify-ochre-tau.vercel.app/)

AI-powered learning platform that turns uploaded PDFs into interactive quizzes and flashcards. It includes AI integrations, cloud document storage, authentication, and Bakong KHQR checkout with server-side payment verification.

`Next.js` `TypeScript` `Groq` `Gemini` `PostgreSQL` `Prisma` `Cloudflare R2`

### MPTC Provincial Website Rebuild

Rebuilt a provincial government website from WordPress to Next.js during my internship at Cambodia's Ministry of Post and Telecommunications, improving page-load speed by **65%**, reducing Time to First Byte by **50%**, and increasing indexed pages by **40%**.

## Recognition

- 🏆 Emerging Innovation Award — November 2025
- 🚀 Unipreneur National Challenge — Top 8 Finalist, October 2025
- 🥉 Youth to Business Hackathon — Second Runner-Up, November 2024
- 🤝 Maybank Student Ambassador — 2025–2026

## Tech stack

<p>
  <img src="https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=next.js&logoColor=white" alt="Next.js" />
  <img src="https://img.shields.io/badge/NestJS-E0234E?style=flat-square&logo=nestjs&logoColor=white" alt="NestJS" />
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white" alt="TypeScript" />
  <img src="https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black" alt="React" />
  <img src="https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white" alt="Node.js" />
  <img src="https://img.shields.io/badge/Laravel-FF2D20?style=flat-square&logo=laravel&logoColor=white" alt="Laravel" />
  <img src="https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white" alt="PostgreSQL" />
  <img src="https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white" alt="MongoDB" />
  <img src="https://img.shields.io/badge/Prisma-2D3748?style=flat-square&logo=prisma&logoColor=white" alt="Prisma" />
  <img src="https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white" alt="Tailwind CSS" />
  <img src="https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white" alt="Docker" />
  <img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white" alt="Git" />
</p>

## GitHub activity

<p>
  <img src="https://streak-stats.demolab.com/?user=KeaMeng-ING&theme=vue-dark&hide_border=true" alt="Kea Meng's GitHub streak" />
</p>

## Contribution snake

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/KeaMeng-ING/new-portfolio/output/github-contribution-grid-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/KeaMeng-ING/new-portfolio/output/github-contribution-grid-snake.svg" />
  <img alt="Kea Meng's contribution snake" src="https://raw.githubusercontent.com/KeaMeng-ING/new-portfolio/output/github-contribution-grid-snake.svg" />
</picture>

## Let's connect

I'm based in **Phnom Penh, Cambodia** and work with teams worldwide. If you'd like to build something together, reach me at **[keameng9@gmail.com](mailto:keameng9@gmail.com)**.
